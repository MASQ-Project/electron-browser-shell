...
export class WindowsAPI {
    ...
    private createWindowDetails(win: BrowserWindow, queryOptions?: any) {
        const tabs = Array.from(this.ctx.store.tabs)
            .filter((tab) => {
    ...
    private getWindowDetails(win: BrowserWindow, queryOptions?: any) {
        // if (this.ctx.store.windowDetailsCache.has(win.id)) {
        //     return this.ctx.store.windowDetailsCache.get(win.id)!;
        // }
        const details = this.createWindowDetails(win, queryOptions);
        return details;
    }
    ...
    private async get(
        ...
        return this.getWindowDetails(win, queryOptions);
    }
